...
class TestCLICommands:
    ...
    def test_cli_help_command(self, python_executable, main_script):
        ...
        self.metrics.record_timing(
            "help_command", self.performance.get_metrics()["duration"]
        )
    ...
    def test_cli_subcommand_help(self, python_executable, main_script):
        ...
        for command in commands:
            ...
            ), f"Help text missing {command} command info"
    ...
    def test_fetch_command_with_limit(self, python_executable, main_script):
        ...
        with TemporaryTestEnvironment() as temp_dir:
            ...
            self.performance.stop()
            self.metrics.record_timing(
                "fetch_command", self.performance.get_metrics()["duration"]
            )
            ...
            # Performance check
            assert (
                self.performance.get_metrics()["duration"] < 30.0
            ), "Fetch command too slow"

    def test_fetch_command_invalid_limit(self, python_executable, main_script):
        """Test fetch command with invalid limit."""
        return_code, stdout, stderr = CommandRunner.run_command(
            [python_executable, main_script, "fetch", "--limit", "-1"]
        )

        # Should handle invalid input gracefully
        assert (
            return_code != 0 or "error" in stderr.lower() or "invalid" in stderr.lower()
        )
    ...
    def test_scrape_command_valid_url(self, python_executable, main_script):
        ...
        return_code, stdout, stderr = CommandRunner.run_command(
            [python_executable, main_script, "scrape", test_url], timeout=30
        )

        self.performance.stop()
        self.metrics.record_timing(
            "scrape_command", self.performance.get_metrics()["duration"]
        )
        ...
        # Performance check
        assert (
            self.performance.get_metrics()["duration"] < 20.0
        ), "Scrape command too slow"
    ...
    def test_scrape_command_invalid_url(self, python_executable, main_script):
        ...
        for url in invalid_urls:
            return_code, stdout, stderr = CommandRunner.run_command(
                [python_executable, main_script, "scrape", url]
            )
    ...
    def test_tts_command_basic(self, python_executable, main_script):
        ...
        with TemporaryTestEnvironment() as temp_dir:
            ...
            return_code, stdout, stderr = CommandRunner.run_command(
                [python_executable, main_script, "tts", test_text, str(output_file)],
                timeout=30,
            )
            ...
            if return_code == 0:
                ...
                is_valid, error = FileValidator.validate_audio_file(
                    output_file, min_size_bytes=100
                )
    ...
    def test_tts_command_empty_text(self, python_executable, main_script):
        ...
        with TemporaryTestEnvironment() as temp_dir:
            ...
            return_code, stdout, stderr = CommandRunner.run_command(
                [python_executable, main_script, "tts", "", str(output_file)]
            )
    ...
    def test_debug_flag(self, python_executable, main_script):
        ...
        return_code, stdout, stderr = CommandRunner.run_command(
            [python_executable, main_script, "--debug", "--help"]
        )
    ...
    def test_config_file_option(self, python_executable, main_script):
        ...
        with TemporaryTestEnvironment() as temp_dir:
            ...
            return_code, stdout, stderr = CommandRunner.run_command(
                [python_executable, main_script, "--config", str(config_file), "--help"]
            )
    ...
    def test_keyboard_interrupt_handling(self, python_executable, main_script):
        ...
        return_code, stdout, stderr = CommandRunner.run_command(
            [python_executable, main_script, "fetch", "--limit", "20"], timeout=1
        )  # Very short timeout
    ...
    def test_concurrent_command_execution(self, python_executable, main_script):
        ...
        def run_help_command():
            return CommandRunner.run_command([python_executable, main_script, "--help"])
    ...
    def test_error_handling_and_exit_codes(self, python_executable, main_script):
        ...
        for cmd_args, description in error_scenarios:
            return_code, stdout, stderr = CommandRunner.run_command(
                [python_executable, main_script] + cmd_args
            )
    ...
    def test_long_running_command_timeout(self, python_executable, main_script):
        ...
        return_code, stdout, stderr = CommandRunner.run_command(
            [
                ...
                "fetch",
                "--limit",
                "50",  # Larger limit might take longer
            ],
            timeout=60,
        )  # 1 minute timeout
        ...
        assert (
            self.performance.get_metrics()["duration"] <= 65
        ), "Command exceeded timeout window"
    ...
    def test_command_line_argument_validation(self, python_executable, main_script):
        ...
        for test_case in test_cases:
            return_code, stdout, stderr = CommandRunner.run_command(
                [python_executable, main_script] + test_case["args"]
            )
